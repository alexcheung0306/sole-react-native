import { ChatMessage } from "@/components/socket/WebSocketService"
...
export const createIndividualChatroom = async (formData) => {
  try {
...
export const getChatroomByUsername = async (receiverName, soleUserId) => {
  try {
    const response = await fetch(
...
export const createChatroomUser = async (formData) => {
  try {
...
export const getAllChatroomBySoleUserId = async (soleUserId) => {
  try {
    const response = await fetch(
