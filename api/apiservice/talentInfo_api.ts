...
export const createTalentInfoWithComcard = async (
  ...
  try {
    ...
    // Append nested talentInfo fields (using request.talentInfo.field pattern)
    ...
    // Only send id for updates (PUT), not for creates (POST)
    if (comcardData.id) {
    ...
    formData.append("comcard.length", comcardData.length?.toString() || "0")
    if (comcardData.pdf) {
      formData.append("comcard.pdf", comcardData.pdf)
    }
    comcardData.photoConfig?.forEach((blob) => {
      if (blob instanceof File) {
        formData.append("comcard.comcardPhotos", blob)
      }
    })

    logFormData("createFormData", formData)
...
export const updateTalentInfoWithComcardBySoleUserId = async ({
  ...
  try {
    ...
    // Append nested comcard fields (using request.comcard.field pattern)
    // Only send id for updates when it exists
    if (comcardData.id) {
      formData.append("comcard.id", comcardData.id.toString())
    }
    formData.append("comcard.configId", "1")
    ...
    // Comcard photo files with position information
    ...
    comcardData.photoConfig?.forEach((blob, actualPosition) => {
      if (blob instanceof File) {
        ...
        console.log(
          `Skipping position ${actualPosition} (existing URL: ${typeof blob === "string" ? blob.substring(0, 50) : blob})`
        )
      }
    })
    console.log(`Total files being sent: ${photoFileIndex}`)
    logFormData("updateFormData", formData)
